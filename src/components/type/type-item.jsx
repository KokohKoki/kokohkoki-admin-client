...
export default function TypeItem({ _id, name, reFetchType }) {
    ...
    <>
      <div className="flex gap-2 items-center text-black font-semibold">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            <DeleteType fishTypeId={_id} onDelete={reFetchType} />
          </div>
        </div>
      </div>
      <div className="w-full h-[2px] bg-gray-500 opacity-50 my-2" />
      <EditType isOpen={isOpen} setIsOpen={setIsOpen} onEdit={reFetchType} typeId={_id} name={name} />
    </>
  );
}
